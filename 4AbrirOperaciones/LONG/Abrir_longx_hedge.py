...
def bajardatosVelaEntrada(moneda_seleccionada, factor, cantidad_factor):
    ...
    if response.status_code == 200:
        ...
        df_total['factor_ATR = 5.5'] = calcular_atr(df_total, period=20)
...
def calcularOrden(client,vela,total_balance, moneda_seleccionada, factor,cantidad_factor): #En esta función se calcula el riesgo de la operación, se establece precio de entrada, precio SL, precio objetivo, apalancamiento.
    ...
    # Verificar que no haya NaN en las columnas 'Precio Máximo' y 'ATR21' para la última vela
    if pd.isna(vela['Precio Máximo'].iloc[-1]) or pd.isna(vela['ATR21'].iloc[-1]):
        print("Valor NaN detectado en las columnas 'Precio Máximo' o 'ATR21'.")
    ...
    while True:
        try:
            ...
            if precio_actual > precioEntrada:
                # Colocar una orden limit post only (posición LONG)
                orden = client.futures_create_order(
                    symbol=symbol,
                    side=SIDE_BUY,                    # Mismo lado (BUY), pero se añade el positionSide
                    positionSide='LONG',              # Indica que es una posición LONG en modo hedge
                ...
                print("     * * *   *   *   *   *   *   *   *   *")
                print("     * Orden limit LONG colocada con éxito *")
                print("     * * *   *   *   *   *   *   *   *   *")
            else:
                # Colocar una orden stop market (posición LONG)
                orden = client.futures_create_order(
                    symbol=symbol,
                    side=SIDE_BUY,
                    positionSide='LONG',              # Indica que es una posición LONG en modo hedge
                ...
                print("     * * *   *   *   *   *   *   *   *   *   *")
                print("     * Orden stop market LONG colocada con éxito *")
                print("     * * *   *   *   *   *   *   *   *   *   *")
