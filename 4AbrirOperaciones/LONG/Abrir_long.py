...
def calcularOrden(client,vela,total_balance, moneda_seleccionada, factor,cantidad_factor): #En esta función se calcula el riesgo de la operación, se establece precio de entrada, precio SL, precio objetivo, apalancamiento.
    apalancamiento = 30        
    precioEntrada = float(vela['Precio Máximo'])
    precioStopLoss = float(vela['Precio Mínimo'])
   ...
   # precioStopLoss = 0.3870  #Precios arbitrarios de prueba
    ...
    TakeProfitprecio = round(precioEntrada*(1+Velax2mascomision),factor)
    ...
    # Verificar si la hora actual está cerca de la medianoche (entre 23:30 y 23:59)
    if hora_actual.hour == 23 and hora_actual.minute >= 30:
        hora_y_minuto_limite = hora_actual.replace(hour=23, minute=59, second=0, microsecond=0).strftime('%H:%M')
    else:
