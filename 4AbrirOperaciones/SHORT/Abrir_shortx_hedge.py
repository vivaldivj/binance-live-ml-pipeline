...
def calcularOrden(client, vela, total_balance, moneda_seleccionada, factor, cantidad_factor):
    apalancamiento = 30
    ...
    # Verificar que no haya NaN en las columnas 'Precio Mínimo' y 'ATR21' para la última vela
    if pd.isna(vela['Precio Mínimo'].iloc[-1]) or pd.isna(vela['ATR21'].iloc[-1]):
        print("Valor NaN detectado en las columnas 'Precio Mínimo' o 'ATR21'.")
        return  # Salir de la función si hay NaN

    # Si todo está bien, proceder con el cálculo
    precioEntrada = float(vela['Precio Mínimo'].iloc[-1])
    atr_actual = vela['ATR21'].iloc[-1]
    ...
    print(f"Precio Entrada: {precioEntrada}, ATR: {atr_actual}, Precio Stop Loss: {precioStopLoss}, Precio Take Profit: {TakeProfitprecio}")
    ...
    while True:
        try:
            ...
            if precio_actual < precioEntrada:
                # Colocar una orden limit post only (posición SHORT)
                orden = client.futures_create_order(
                    symbol=symbol,
                    side=SIDE_SELL,                   # Dirección de la orden
                    positionSide='SHORT',             # Indica que es una posición SHORT en modo hedge
                ...
                print("     * * *   *   *   *   *   *   *   *   *")
                print("     * Orden limit SHORT colocada con éxito *")
                print("     * * *   *   *   *   *   *   *   *   *")
            else:
                # Colocar una orden stop market (posición SHORT)
                orden = client.futures_create_order(
                    symbol=symbol,
                    side=SIDE_SELL,
                    positionSide='SHORT',             # Indica que es una posición SHORT en modo hedge
                ...
                print("     * * *   *   *   *   *   *   *   *   *   *")
                print("     * Orden stop market SHORT colocada con éxito *")
                print("     * * *   *   *   *   *   *   *   *   *   *")
