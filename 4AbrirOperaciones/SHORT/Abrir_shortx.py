...
def calcularOrden(client, vela, total_balance, moneda_seleccionada, factor, cantidad_factor):
    ...
    # Verificar que no haya NaN en las columnas 'Precio Mínimo' y 'ATR20' para la última vela
    if pd.isna(vela['Precio Mínimo'].iloc[-1]) or pd.isna(vela['ATR20'].iloc[-1]):
        print("Valor NaN detectado en las columnas 'Precio Mínimo' o 'ATR20'.")
