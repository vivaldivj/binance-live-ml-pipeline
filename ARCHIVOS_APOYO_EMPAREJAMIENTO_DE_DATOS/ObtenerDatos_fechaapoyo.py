...
# 1.- Descargar los historicos de las principales criptomonedas: Bitcoin, Etherium, Binance Coin, Ripple, Bitcoincash, Eterium Classic, Doge (desde enero del 2021)
# 2.- Generar un archivo EXCEL con los datos historicos descargados de cada criptomoneda
import requests
import pandas as pd
...
def crearDataFrame(moneda_seleccionada,factor, eventos_confirmados):
    ...
    # Solicitar los datos de fecha y hora al usuario
    anio = int(input("Ingresa el año (formato YYYY): "))
    mes = int(input("Ingresa el mes (formato MM): "))
    dia = int(input("Ingresa el día (formato DD): "))
    hora = int(input("Ingresa la hora (formato 24h HH): "))
    minuto = int(input("Ingresa el minuto (formato mm): "))

    # Crear un objeto datetime con los valores proporcionados
    current_time = datetime(anio, mes, dia, hora, minuto, 0)  # Segundos y microsegundos en 0
    saved_time = current_time
    # Imprimir el resultado
    print(f"La fecha ingresada es: {current_time}")
    ...
    start_time = int((end_time - timedelta(minutes=30 * 1000)).timestamp() * 1000)   ################ SIEMPRE AJUSTA timedelta(minutes=30 * 200). 200 ES EL NUMERO DE VELAS QUE PIDES
...
# Reordenar las columnas
    ...
    df_total['SMA20'] = ta.hma(df_total['Precio de Cierre'], length=20).round(2)
    df_total['SMA8'] = ta.sma(df_total['Precio de Cierre'], length=8).round(2)
    df_total['SMA4'] = ta.sma(df_total['Precio de Cierre'], length=4).round(2)
    ...
    # Estas dos columnas son auxiliares, se van a eliminar 
    df_total['CuerpoGreen'] = np.where(df_total['Color'] == 1, df_total['Cuerpo2'], np.nan) # 
    df_total['CuerpoRed'] = np.where(df_total['Color'] == -1, df_total['Cuerpo2'], np.nan) # 
    #   #   #
    df_total['CuerpoGreen'] = df_total['CuerpoGreen'].fillna(0)
    df_total['CuerpoRed'] = df_total['CuerpoRed'].fillna(0)
    df_total['Cuerpo_AVG_GREEN'] = ta.sma(df_total['CuerpoGreen'], length=30).round(0) 
    df_total['Cuerpo_AVG_RED'] = ta.sma(df_total['CuerpoRed'], length=30).round(0)   

    # Pendientes de SMAs
    df_total['PendienteSMA4'] = getPendienteSMA4(df_total)
    df_total['PendienteSMA8'] = getPendienteSMA8(df_total)
    df_total['PendienteSMA20'] = getPendienteSMA20(df_total)

    # Midiendo el nivel de poder del color
    df_total['LVL_Pwr_Green'] = LVL_Pwr_Green(df_total)
    df_total['LVL_Pwr_Red'] = LVL_Pwr_Red(df_total)

    #Confirmación de velas de de poder
    ...
    df_total['rebote_verde']= rebote_verde(df_total)
    df_total['rebote_rojo']= rebote_rojo(df_total)   
    df_total['DV_Alcista']= DV_Alcista(df_total)
    df_total['DV_Bajista']= DV_Bajista(df_total)  

    # Mecha alcista vs Mecha bajista
    df_total['Mecha_alcistavsbajista'] = mechaganadora(df_total).round(0) 


    # Usando BINS
    df_total['RSI_50bins']= RSI_50bins(df_total)
    df_total['MechaAlcista_50bins']= MechaAlcista_50bins(df_total) 
    df_total['MechaBajista_50bins']= MechaBajista_50bins(df_total) 
    df_total['Cuerpo_50bins']= Cuerpo_50bins(df_total)
    ...
    df_total['Volatilidad'] = (obtenerVolatilidad_Por_Vela(df_total)).round(2) 
    df_total['Elefanteverde'] = Elefanteverde(df_total).round(0)   
    df_total['Elefanterojo'] = Elefanterojo(df_total).round(0)

    df_total['Cierre'] = (Cierre(df_total)).round(0)  
    

    df_total['Contc_20'] = Contc_20(df_total).round(0)   
    df_total['Contc_8'] = Contc_8(df_total).round(0)                             
    df_total['Contc_4'] = Contc_4(df_total).round(0)

    df_total['Verde_mas_grande'] = (la_mayor_verde(df_total)).round(0)  
    df_total['Roja_mas_grande'] = (la_mayor_roja(df_total)).round(0)    

    #    D   R   O   P   S       D   E       C   R   E   A   R       D   A   T   A   S   E   T   S       #
    df_total = df_total.drop(columns=['Volumen'])
    df_total = df_total.drop(columns=['CuerpoGreen'])  
    df_total = df_total.drop(columns=['CuerpoRed'])
    df_total = df_total.drop(columns=['Cuerpo_AVG_GREEN'])  
    df_total = df_total.drop(columns=['Cuerpo_AVG_RED'])  
    ...
    df_total = df_total.drop(columns=['SMA4']) 
    ...
    df_total = df_total.drop(columns=['Volatilidad'])  

    vela_mas_grande_verdeconfirm = df_total['Elefanteverde'].iloc[-1]
    print("Elefante verde?: ", vela_mas_grande_verdeconfirm)
    vela_mas_grande_rojaconfirm = df_total['Elefanterojo'].iloc[-1]
    print("Elefante rojo?: ",vela_mas_grande_rojaconfirm)
    # Antes de irnos, si el patrón se confirmó, antes de enviar la confirmación al main, checamos que la vela de entrada no sea la más grande del dataframe O una elefante
    if (patron_180_Buy == True) or (patron_RSI50_Buy == True) or (patron_HAMMER_Buy == True) or (patron_DV_Buy == True):      
        if (vela_mas_grande_verdeconfirm == 1).any():
            print(" Se ha identificado un posible evento de entrada, pero la vela de entrada es elefante verde. Se evitará entrada...")

            eventos_confirmados.clear()

    if (patron_180_Sell == True) or (patron_RSI50_Sell == True) or (patron_HAMMER_Sell == True) or (patron_DV_Sell == True):
        if (vela_mas_grande_rojaconfirm == 1).any():
            print(" Se ha identificado un posible evento de entrada, pero la vela de entrada es elefante rojo. Se evitará entrada...")

            eventos_confirmados.clear()

    return df_total,eventos_confirmados, patron_180_Buy, patron_180_Sell, patron_RSI50_Buy, patron_RSI50_Sell, patron_HAMMER_Buy, patron_HAMMER_Sell, patron_DV_Buy, patron_DV_Sell, saved_time
...
def crearDataFrameLARGE(current_time,moneda_seleccionada,factor):
    print(f"El current time recibido en la fnución large es {current_time}")
    # Define los parámetros de la solicitud
    symbol = f'{moneda_seleccionada}USDT' ################## <-
    ##################################### <-
    interval = '6h'
    ...
    if current_time.hour >= 0 and current_time.hour < 6:
        end_time = (current_time - timedelta(days=1)).replace(hour=18, minute=0, second=0, microsecond=0)
    elif current_time.hour >= 6 and current_time.hour < 12:
    ...
    elif current_time.hour >= 18 and current_time.hour < 24:
    ...
    start_time = end_time - timedelta(hours=6 * (limit - 1))
...
def RSI_50bins(df_total):
    ...
    bin_edges = [8 + i * ((91 - 8) / 50) for i in range(51)]  # 51 puntos para 50 bins
...
#                    Funciones de apoyo para normalizar y/o crear columnas nuevas antes de etiquetarlas y juntarlas
...
def Volatilidad_50bins(rsi_value, df_total):
    """
    Asigna un bin (1 a 50) al valor del RSI basado en el rango dinámico del dataframe.
    
    Args:
        rsi_value (float): Valor del RSI para la fila actual.
        df_total (pd.DataFrame): DataFrame que contiene la columna RSI.
        
    Returns:
        int: Número del bin (1 a 50) al que pertenece el valor de RSI.
    """
    # Obtener el rango dinámico del RSI
    rsi_min = df_total['Volatilidad'].min()
    rsi_max = df_total['Volatilidad'].max()

    # Manejar casos donde todo el RSI es igual (para evitar división por cero)
    if rsi_max == rsi_min:
        return 1

    # Dividir el rango en 50 bins
    bin_edges = [rsi_min + i * ((rsi_max - rsi_min) / 50) for i in range(51)]  # 51 puntos para 50 bins

    # Asignar el valor de RSI al bin correspondiente
    for i in range(1, len(bin_edges)):
        if bin_edges[i - 1] <= rsi_value < bin_edges[i]:
            return i

    # Si por alguna razón el valor de RSI es igual al máximo, asignarlo al último bin
...
def patron_confirmacion180_Buy(datos_current):
    ...
    if (datos_current['Toro_180'].iloc[-1] == 1) and (datos_current['RSI_Alcista'].iloc[-1] == 0)and (datos_current['Martillo_Alcista'].iloc[-1] == 0):
                print(" Se detecto Toro_180")
                patron_confirmacion180_Buy = True
    else:
        print("No hay 180 BUY por el momento.")  
    return patron_confirmacion180_Buy    

def patron_confirmacion180_Sell(datos_current):
    patron_confirmacion180_Sell = False
    if (datos_current['Oso_180'].iloc[-1] == 1) and (datos_current['RSI_Bajista'].iloc[-1] == 0)and (datos_current['Martillo_Bajista'].iloc[-1] == 0):
            print(" Se detecto Oso_180")
            patron_confirmacion180_Sell = True
    else:
        print("No hay 180 SELL por el momento.") 
...
def patron_confirmacion_RSI50_Buy(datos_current):
    ...
    if (datos_current['RSI_Alcista'].iloc[-1] == 1):
        ...
        print("No hay RSI50 BUY por el momento.")  
    return patron_confirmacion180_Buy    
...
def patron_confirmacion_RSI50_Sell(datos_current):
    ...
    if (datos_current['RSI_Bajista'].iloc[-1] == 1):
        ...
        print("No hay RSI50 SELL por el momento.") 
    return patron_confirmacion180_Sell 
...
def patron_confirmacion_HAMMER_Buy(datos_current):
    patron_confirmacion180_Buy = False    
    if  (datos_current['Martillo_Alcista'].iloc[-1] == 1)and(datos_current['Toro_180'].iloc[-1] == 0) and (datos_current['RSI_Alcista'].iloc[-1] == 0):
                print(" Se detecto Martillo_Alcista")
                patron_confirmacion180_Buy = True
    else:
        print("No hay HAMMER BUY por el momento.")  
    return patron_confirmacion180_Buy    

def patron_confirmacion_HAMMER_Sell(datos_current):
    patron_confirmacion180_Sell = False
    if  (datos_current['Martillo_Bajista'].iloc[-1] == 1)and(datos_current['Oso_180'].iloc[-1] == 0) and (datos_current['RSI_Bajista'].iloc[-1] == 0):
            print(" Se detecto Martillo_Bajista")
            patron_confirmacion180_Sell = True
    else:
        print("No hay HAMMER SELL por el momento.") 
    return patron_confirmacion180_Sell 
...
def patron_confirmacion_DV_Buy(datos_current):
    patron_confirmacion180_Buy = False    
    if  (datos_current['Precio de Cierre'].iloc[-1] > datos_current['Precio Máximo'].iloc[-2]) and(datos_current['Toro_180'].iloc[-1] == 0) and (datos_current['RSI_Alcista'].iloc[-1] == 0)and (datos_current['Martillo_Alcista'].iloc[-1] == 0):
                print(" Se detecto DV_Alcista")
                patron_confirmacion180_Buy = True
    else:
        print("No hay DV BUY por el momento.")  
    return patron_confirmacion180_Buy    

def patron_confirmacion_DV_Sell(datos_current):
    patron_confirmacion180_Sell = False
    if  (datos_current['Precio de Cierre'].iloc[-1] < datos_current['Precio Mínimo'].iloc[-2]) and(datos_current['Oso_180'].iloc[-1] == 0) and (datos_current['RSI_Bajista'].iloc[-1] == 0)and (datos_current['Martillo_Bajista'].iloc[-1] == 0):
            print(" Se detecto DV_Bajista")
            patron_confirmacion180_Sell = True
    else:
        print("No hay DV SELL por el momento.") 
    return patron_confirmacion180_Sell 
...
if __name__ == "__main__":
    ...
    datos,eventos_confirmados,confirmacion180BUY, confirmacion180SELL, patron_RSI50_Buy, patron_RSI50_Sell, patron_HAMMER_Buy, patron_HAMMER_Sell, patron_DV_Buy, patron_DV_Sell, current_time = crearDataFrame(moneda_seleccionada,factor, eventos_confirmados)
    ...
    datosLarge = crearDataFrameLARGE(current_time,moneda_seleccionada,factor)
    # Guardamos todos los datos en un archivo de CSV
    ...
    if eventos_confirmados:  # Se ejecutará solo si la lista no está vacía
        ...
        ruta_script2 = "2Aplanar/Aplanador.py"         
            ...
            if evento == "Toro_180":
                ...
                try:
                    ...
                    result_script2 = subprocess.Popen(
                        ["python", ruta_script2, moneda_seleccionada, str(evento_entrada)]
                    )
